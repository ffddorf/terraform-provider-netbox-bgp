...
- `api_token` (String, Sensitive) Netbox API authentication token. Can be set via the `NETBOX_API_TOKEN` environment variable.
- `ca_cert_file` (String) CA certificate bundle to use for verification of Netbox server certificate. Can be set via the `NETBOX_CA_CERT_FILE` environment variable.
- `headers` (Map of String, Sensitive) Set these header on all requests to Netbox. Can be set via the `NETBOX_HEADERS` environment variable in the format `header-a:value-a,header-b:value-b`
